...
export default function BankSoal() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const normalizeCategory = (cat: any) => {
        const str = (cat || '').trim();
        if (!str || str.toLowerCase() === 'umum') {
          return teacherSubjects[0] || 'Informatika';
        }
        return str;
      };
  ...
  const syncToDrive = async (updatedQuestions: any[]) => {
    const normalized = updatedQuestions.map(q => ({
      ...q,
      category: (!q.category || q.category.trim().toLowerCase() === 'umum') ? (teacherSubjects[0] || 'Informatika') : q.category.trim(),
  ...
  const handleTypeChange = (type: string) => {
    ...
    if (type === 'Pilihan Ganda Asosiatif (TKA)') {
      ...
      extra = {
        ...
        jawaban_benar: type === 'Pilihan Ganda Kompleks' ? 'a' : (newQuestion.jawaban_benar || 'a')
      };
    } else if (type === 'Menjodohkan') {
      extra = {
        option_a: newQuestion.option_a.includes('=') ? newQuestion.option_a : 'Premis 1 = Pasangan 1',
        option_b: newQuestion.option_b.includes('=') ? newQuestion.option_b : 'Premis 2 = Pasangan 2',
        option_c: newQuestion.option_c.includes('=') ? newQuestion.option_c : 'Premis 3 = Pasangan 3',
        option_d: newQuestion.option_d.includes('=') ? newQuestion.option_d : 'Premis 4 = Pasangan 4',
        option_e: '',
        jawaban_benar: 'auto'
      };
      ...
      extra = {
        ...
        option_d: '',
        option_e: '',
        jawaban_benar: newQuestion.jawaban_benar && newQuestion.jawaban_benar.length > 2 ? newQuestion.jawaban_benar : ''
      };
    } else if (type === 'Drag and Drop') {
      extra = {
        option_a: newQuestion.option_a || 'Tahap 1 (Awal)',
        option_b: newQuestion.option_b || 'Tahap 2',
        option_c: newQuestion.option_c || 'Tahap 3',
        option_d: newQuestion.option_d || 'Tahap 4 (Akhir)',
        option_e: '',
  ...
  const downloadTemplate = () => {
    const templateData = [
      ...
      {
        'Pertanyaan': 'Manakah dari pernyataan berikut yang merupakan ciri-ciri makhluk hidup? (Pilih lebih dari satu)',
        'Tipe': 'Pilihan Ganda Kompleks',
        'Kategori': 'Biologi',
        'Opsi A': 'Bernapas',
        'Opsi B': 'Tidak bergerak',
        'Opsi C': 'Berkembang biak',
        'Opsi D': 'Peka terhadap rangsang',
        'Opsi E': 'Tidak memerlukan nutrisi',
        'Jawaban Benar': 'a,c,d',
        'Link Gambar': ''
      },
      {
        'Pertanyaan': 'Jodohkan nama negara berikut dengan ibukotanya yang sesuai!',
        'Tipe': 'Menjodohkan',
        'Kategori': 'Geografi',
        'Opsi A': 'Indonesia = Jakarta',
        'Opsi B': 'Jepang = Tokyo',
        'Opsi C': 'Prancis = Paris',
        'Opsi D': 'Inggris = London',
        'Opsi E': '',
        'Jawaban Benar': 'auto',
        'Link Gambar': ''
      },
      {
        ...
        'Opsi D': '',
        'Opsi E': '',
        'Jawaban Benar': 'Fotosintesis',
        'Link Gambar': ''
      },
      {
        'Pertanyaan': 'Urutkan tahapan metamorfosis sempurna pada kupu-kupu dari awal hingga akhir!',
        'Tipe': 'Drag and Drop',
        'Kategori': 'Biologi',
        'Opsi A': 'Telur',
        'Opsi B': 'Ulat (Larva)',
        'Opsi C': 'Kepompong (Pupa)',
        'Opsi D': 'Kupu-kupu dewasa (Imago)',
        'Opsi E': '',
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const newQuestions = data.map((row: any, idx: number) => {
          let type = row['Tipe'] || row['type'] || 'Pilihan Ganda';
          const typeLower = String(type).toLowerCase();
          
          if (typeLower.includes('asosiatif')) {
            type = 'Pilihan Ganda Asosiatif (TKA)';
          } else if (typeLower.includes('sebab') || typeLower.includes('akibat')) {
            type = 'Hubungan Sebab Akibat (TKA)';
          ...
          if (type === 'Pilihan Ganda Asosiatif (TKA)' && (!option_a || option_a.trim() === '')) {
            ...
            option_d = 'Pernyataan salah dan alasan benar';
            option_e = 'Pernyataan dan alasan keduanya salah';
          } else if (type === 'Menjodohkan' && (!jawaban_benar || jawaban_benar === 'a')) {
            jawaban_benar = 'auto';
          }
  ...
  const handleBatchMove = async (targetFolder: string) => {
    ...
    const updatedQuestions = questions.map(q => {
      if (selectedQuestions.includes(q.id)) {
        return { ...q, category: targetFolder };
      }
      return q;
    });
  ...
  const filteredQuestions = jenjangFilteredQuestions.filter(q => {
    ...
    if (activeFolder !== null) {
      const cat = (q.category || '').toLowerCase().trim();
      return cat === activeFolder.toLowerCase() || cat.includes(activeFolder.toLowerCase()) || activeFolder.toLowerCase().includes(cat);
    }
    return true;
  });
    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="tracking-tight">{activeFolder !== null ? `Mata Pelajaran: ${activeFolder}` : 'Bank Soal'}</h2>
          <p className="text-slate-500 text-sm font-medium">
            {activeFolder !== null 
              ? `Koleksi soal untuk mata pelajaran ${activeFolder}.` 
              : 'Koleksi pertanyaan ujian Anda yang tersimpan di Cloud berdasarkan mata pelajaran.'}
          </p>
        </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            <div className="bg-blue-50 text-blue-600 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border border-blue-100">
                              {q.category || teacherSubjects[0] || 'Informatika'}
                            </div>
                          </div>
                        </div>
        ...
        >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {(newQuestion.type === 'Pilihan Ganda' || newQuestion.type === 'Pilihan Ganda Asosiatif (TKA)' || newQuestion.type === 'Hubungan Sebab Akibat (TKA)') && (
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-bold text-slate-700">Opsi Jawaban & Kunci</label>
                    <span className="text-[10px] text-slate-400 font-bold">Klik lingkaran huruf untuk menentukan kunci jawaban benar</span>
                  </div>
                  {[
              ...
              {newQuestion.type === 'Pilihan Ganda Kompleks' && (
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-bold text-slate-700">Opsi Jawaban (Multi-Kunci)</label>
                    <span className="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-md">Centang semua kunci jawaban yang benar</span>
                  </div>
                  {[
                    { key: 'option_a', label: 'A', id: 'a' },
                    { key: 'option_b', label: 'B', id: 'b' },
                    { key: 'option_c', label: 'C', id: 'c' },
                    { key: 'option_d', label: 'D', id: 'd' },
                    { key: 'option_e', label: 'E', id: 'e' }
                  ].map((opt) => {
              ...
              {newQuestion.type === 'Menjodohkan' && (
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-bold text-slate-700">Pasangan Jawaban (Kiri = Kanan)</label>
                    <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-md">Format: [Premis/Pernyataan] = [Pasangan/Jawaban]</span>
                  </div>
                  <p className="text-[11px] text-slate-400">Tuliskan pasangan yang cocok pada masing-masing baris. Sistem akan otomatis memasangkan dan mengacak sisi kanan saat ujian.</p>
                  {[
                    { key: 'option_a', label: '1' },
                    { key: 'option_b', label: '2' },
                    { key: 'option_c', label: '3' },
                    { key: 'option_d', label: '4' },
                    { key: 'option_e', label: '5 (Opsional)' }
                  ].map((opt) => {
                    const rawVal = String(newQuestion[opt.key as keyof typeof newQuestion] || '');
                    const parts = rawVal.split('=');
                    const leftVal = parts[0]?.trim() || '';
                    const rightVal = parts.slice(1).join('=').trim();
                    return (
                      <div key={opt.key} className="flex flex-col sm:flex-row items-center gap-2 bg-slate-50 p-2.5 rounded-2xl border border-slate-100">
                        <span className="w-6 h-6 rounded-lg bg-indigo-100 text-indigo-950 font-bold text-xs flex items-center justify-center shrink-0">{opt.label}</span>
                        <input
                          type="text"
                          placeholder="Pernyataan Kiri (Premis)..."
                          className="flex-1 w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none text-xs font-semibold text-indigo-950"
                          value={leftVal}
                          onChange={(e) => {
                            const newCombined = e.target.value.trim() || rightVal ? `${e.target.value} = ${rightVal}` : '';
                            setNewQuestion({ ...newQuestion, [opt.key]: newCombined });
                          }}
                        />
                        ...
                        <input
                          ...
                          className="flex-1 w-full px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none text-xs font-semibold text-emerald-700"
                          value={rightVal}
                          onChange={(e) => {
                            const newCombined = leftVal || e.target.value.trim() ? `${leftVal} = ${e.target.value}` : '';
                            setNewQuestion({ ...newQuestion, [opt.key]: newCombined });
                          }}
                        />
                      </div>
              ...
              {newQuestion.type === 'Isian Singkat' && (
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-bold text-slate-700">Kunci Jawaban Teks / Angka</label>
                    <span className="text-[10px] text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded-md">Case-insensitive (Toleran Huruf Besar/Kecil)</span>
                  </div>
                  <input
                    type="text"
                    placeholder="Contoh: Fotosintesis atau 25 atau alternatif: Jakarta|DKI Jakarta"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 outline-none font-bold text-xs text-indigo-950 focus:border-indigo-950"
              ...
              {newQuestion.type === 'Drag and Drop' && (
                <div className="space-y-3 pt-2">
                  ...
                  {[
                    ...
                    { key: 'option_c', label: 'Tahap 3' },
                    { key: 'option_d', label: 'Tahap 4' },
                    { key: 'option_e', label: 'Tahap 5 (Opsional)' }
                  ].map((opt) => (
                    <div key={opt.key} className="flex items-center gap-3">
                      <span className="w-16 text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0">{opt.label}</span>
                      <input
                        type="text"
                        placeholder={`Masukkan isi ${opt.label}...`}
                        className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 outline-none text-xs font-medium text-indigo-950"
                        value={newQuestion[opt.key as keyof typeof newQuestion]}
                        onChange={(e) => setNewQuestion({ ...newQuestion, [opt.key]: e.target.value })}
                      />
            ...
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-100/80 flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-[9px] text-slate-400 leading-normal mt-0.5">
                  {autoDetectCategory 
                    ? "Membaca kolom/baris Kategori dari berkas impor untuk mencocokkan nama mata pelajaran."
                    : `Menyimpan semua soal impor ke folder mata pelajaran aktif (${activeFolder || teacherSubjects[0] || 'Informatika'}).`
                  }
                </p>
              </div>
